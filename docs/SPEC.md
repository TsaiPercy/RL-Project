...
自行訓練 agent，使用 BabyAI 環境 curriculum（room_size=15），7×7 partially observable 視野。強弱 agent 差異透過**成功率門檻**與**訓練環境種類**製造。
...
- **Strong agent**：通過完整 curriculum（所有 8 關），門檻設定較高（如 85-90%）
- **Weak agent**：僅通過部分 curriculum（如前 3-4 關），或門檻設定較低（如 50-60%）
- 透過調整門檻與訓練關卡數量，可靈活製造不同程度的強弱差異
...
- **來源**：自行訓練（使用 BabyAI 環境 curriculum，room_size=15）
- **Observation**：7×7 partially observable grid
- **Strong agent**：通過完整 curriculum（GoToObj → ... → BossLevel），高成功率門檻
- **Weak agent**：僅通過部分 curriculum 或使用較低成功率門檻
- **強弱差異機制**：透過調整 config 中各關卡成功率門檻及訓練的環境種類數量
...
agent_training:
  ...
  curriculum:                         # ordered from easy to hard
    - env: "BabyAI-GoToObj-v0"
      success_threshold: 0.80         # advance to next when reaching this rate
